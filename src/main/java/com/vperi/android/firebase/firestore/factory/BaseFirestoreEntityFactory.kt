...
protected constructor() :
  ...
  override fun createInstance(
      ...
      createInstance(
          when (id) {
            null -> collection.document()
            else -> collection.document(id)
          })

  override fun createInstance(doc: DocumentReference): P<T> {
    return doc.get().promise.then {
      val inst = this.createInstance(it)
      inst.id = it.id
      inst
    }
  }
